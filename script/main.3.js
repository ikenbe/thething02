...
function goLeft(){
    ...
    for(var x=0;x<4;x+=1){
        for(var y=0;y<3;y+=1){
            if (cellMoving[x][y]==1){ gridNow[x][y]=gridNow[x][y+1];cellMoving[x][y+1]=1;}
            else if (gridNow[x][y]===0){ gridNow[x][y]=gridNow[x][y+1];cellMoving[x][y+1]=1;}
            else if((gridNow[x][y]==gridNow[x][y+1])&&(gridNow[x][y]>2)){gridNow[x][y]+=gridNow[x][y];cellMoving[x][y+1]=1;}
            else if((gridNow[x][y]+gridNow[x][y+1]==3)&&gridNow[x][y]<3){gridNow[x][y]=3;cellMoving[x][y+1]=1;}
            else cellMoving[x][y+1]=0;
        }
        if (cellMoving[x][3]==1){ gridNow[x][3]=0;}
    }
}
...
function goRight(){
    ...
    for(var x=0;x<4;x+=1){
        ...
        if (cellMoving[x][0]==1){ gridNow[x][0]=0; }
    }
}
...
function goUp(){
    ...
    for(var y=0;y<4;y+=1){
        for(var x=0;x<3;x+=1){
            if (cellMoving[x][y]==1){ gridNow[x][y]=gridNow[x+1][y];cellMoving[x+1][y]=1;}
            else if (gridNow[x][y]===0){ gridNow[x][y]=gridNow[x+1][y];cellMoving[x+1][y]=1;}
            else if((gridNow[x][y]==gridNow[x+1][y])&&(gridNow[x][y]>2)){gridNow[x][y]+=gridNow[x][y];cellMoving[x+1][y]=1;}
            else if((gridNow[x][y]+gridNow[x+1][y]==3)&&gridNow[x][y]<3){gridNow[x][y]=3;cellMoving[x+1][y]=1;}
            else cellMoving[x+1][y]=0;
        }
        if (cellMoving[3][y]==1){ gridNow[3][y]=0; }
    }
}
...
function goDown(){
    ...
    for(var y=0;y<4;y+=1){
        ...
        if (cellMoving[0][y]==1){ gridNow[0][y]=0; }
    }
}
...
function gridMove(direction){
    ...
    setTimeout(transData(),500);//传递变化量
...
function resetArray(array){
    for( var i=0;i<array.length;i+=1 ){
        for( var j=0;j<array[i].length;j+=1 ){ array[i][j] = 0; }
    }
}
...
//    console.log(cellMoving);
    if (D=="left"){
        for(i=0;i<4;i+=1){
            for(j=0;j<4;j+=1){
                if(cellMoving[i][j]){
                    $('.cell').eq((j*4)+i).css({left:$g*(i%4+1)}).animate({
                        left:$g*(i%4),
                        color:'red',
                    },300);
                }
        ...
        for(i=0;i<4;i+=1){
            for(j=0;j<4;j+=1){
                if(cellMoving[i][j]){
                    $('.cell').eq((j*4)+i+1).css({left:$g*(i%4-1)}).animate({
                        left:$g*(i%4),
                        color:'red',
                    },300);
                }
